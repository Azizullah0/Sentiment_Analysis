...
\IfFontExistsTF{Amiri}{%
  ...
  \newfontfamily\persianfont[Script=Arabic,Scale=1.05]{Tahoma}%
}
\newcommand{\dariit}[1]{\textit{\begin{persian}#1\end{persian}}}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{graphicx}
...
\begin{itemize}
    \item \dariit{ای خدا رحم کن، پاکستان هر لحظه ممکن است حمله کند، خدا رحم کنه}
    \item \dariit{می‌ترسم طالبان دوباره مردم را بکشند}
    \item \dariit{نگران آیندهٔ کودکان هستم که همه چیز خراب‌تر شود}
\end{itemize}
...
Examples:
\begin{itemize}
    \item \dariit{وای! قیمت‌ها یک‌شبه تغییر کرد}
    \item \dariit{چطور ممکن است حکومت دوباره وعدهٔ میان‌تهی بدهد؟}
    \item \dariit{باورم نمی‌شود، بازار واقعاً سقوط کرد}
\end{itemize}
...
Examples:
\begin{itemize}
    \item \dariit{این مسئولین مردم را فراموش کرده‌اند}
    \item \dariit{به خدا قسم، زورمندان دیگر بس است}
    \item \dariit{چرا ادارهٔ برق هیچ کاری نمی‌کند؟}
\end{itemize}
...
Examples:
\begin{itemize}
    \item \dariit{اف به این روزگار، این فساد اداری قابل تحمل نیست}
    \item \dariit{این رشوت‌خوری در این کشور شرم‌آور است}
    \item \dariit{از این دورویی بیزارم، حال آدم را بد می‌کند}
\end{itemize}
...
To check that the A4 result was not a single lucky training run, I trained A4 five times with training seeds 41--45 while keeping the stratified train--test split fixed at \texttt{random\_state=42}. Each run writes to a separate output directory (\texttt{seed\_41} through \texttt{seed\_45}); the original anchor model at \texttt{outputs/ablation/A4/} is unchanged. I aggregated metrics with \texttt{scripts/run\_multiseed.py} (summary in \texttt{multiseed\_summary.json}).

Table~\ref{tab:multiseed} reports per-seed accuracy, Macro-F1, and Fear F1 on the same held-out 20\%. Across seeds, accuracy ranged from 86.10\% to 86.22\% and Macro-F1 from 0.856 to 0.858. The anchor A4 result (86.12\%, 0.857, 0.950 Fear F1) falls within this spread. Mean Macro-F1 varied by only $\pm 0.001$, so the ablation result is stable under training randomness.

\begin{table}[H]
\centering
\caption{A4 multi-seed results (fixed split \texttt{random\_state=42}, training seeds 41--45).}
\label{tab:multiseed}
\begin{tabular}{@{}rccc@{}}
\toprule
...
\midrule
Mean $\pm$ std & 86.16\% $\pm$ 0.05\% & 0.857 $\pm$ 0.001 & 0.947 $\pm$ 0.003 \\
\bottomrule
\end{tabular}
\end{table}
